...
This reference is a combination of several parts:

1. **Registry**: This is akin to the home or the server where your images or artifacts are stored.
Think of it as the overarching domain. Examples include Docker Hub,
Google Container Registry, or your own private registry.
2. **Repository**: This is a namespace under the registry where a particular 
...
3. **Tag or Digest**: This is the version of the image or artifact you want to pull or push.

#### References by Tag

- **Tag**: Tags are user-friendly references to specific image versions.
Common tags include `latest`, `v1.0`,
or any other description that makes sense for versioning or identiying.
...
:::caution
Tags are mutable. This means that the same tag can point to different images over time.
:::
...
![Diagram showing example of by tag reference](/img/concepts/reference/by_digest.png)
...
Putting it all together, a reference includes a registry, repository and a tag or a digest in the context
of a **registry**.
...
using the reference `nginx:latest`, Docker would automatically resolve this to
`docker.io/library/nginx:latest`. Here, `docker.io` is the default registry and
`library` is the default repository for official images.

### Conclusion

Understanding the components of an `reference` is vital when working with containers or OCI artifacts.
These references ensure that clients are addressing the desired version of the iamge or artifact.
...
[oci-layout-index]: https://github.com/opencontainers/image-spec/blob/main/image-layout.md#indexjson-file