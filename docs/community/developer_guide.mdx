...
2. Open a PR to update Github Actions with latest version of golang: replace go version of [binary](https://github.com/oras-project/oras/blob/main/.github/workflows/release-github.yml#L32) and [image](https://github.com/oras-project/oras/blob/main/Dockerfile#L14) to latest stable one
...
3. (optional) Cut off a release branch named `release-<major>.<minor>` **ONLY** when releasing a new minor version
...
1. Validate the signatures. Not that the `KEYS` file should be imported with `gpg --import KEYS`. Run some form of the following (adapted from Linux project):
    ...
    ```
1. Click "Edit release" button on the release, and add the `.asc` files created in the previous step. Edit the release description to include 1) vote result, 2) change logs and 3) a note indicating releaser's GPG key used to sign the artifacts. Example (replace with releaser's fingerprint etc.):
    ```
    ## Notes
