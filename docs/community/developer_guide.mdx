...
3. (patch-release-only) Cherry-pick the fix commits to the patch release branch.
...
1. Validate the signatures. Note that the `KEYS` file should be imported with `gpg --import KEYS`. Run some form of the following (adapted from Linux project):
    ...
    ```
1. Click the "Edit release" button on the release, and add the `.asc` files created in the previous step. Edit the release description to include change logs and a note indicating releaser's GPG key used to sign the artifacts. Example (replace with releaser's fingerprint etc.):
    ```
    ## Notes
