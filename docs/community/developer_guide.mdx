...
1. Send a message to slack channel to call for vote on cutting off a release branch named `release-<major>.<minor>` based on the version update commit. If the vote passes, merge the PR.
...
### Release
1. Make fresh clone of the repo after all above steps are completed. Create a new tag for the version prefixed with "v" and push the tag directly to the repo. The tag should be created on the target commit, **NOT** the merge commit.
    ```sh
    version=1.0.0
    git tag v${version}
