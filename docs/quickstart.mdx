...
We will be using [zot registry](https://zotregistry.io/) as our example for a local registry 
and [Docker Hub](https://hub.docker.com/) for remote. 
...
However, you can refer to their [installation guide](https://zotregistry.io/install-guides/install-guide-linux/) 
...
Although it is not necessary to authenticate for all local registries (including zot registry),
you will need to authenticate for remote registries.
You can use [`oras login`](./commands/oras_login.mdx) for the same.

**Note:** You will need to replace `localhost:5000` with the location of the remote registry.
For example for Docker Hub, 
you may replace `localhost:5000/hello-artifact:v1` with `docker.io/username/registry-name:tag`.
