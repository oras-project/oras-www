...
Check out the user guidance [Backup and Restore of OCI Artifacts, Images, and Repositories](/docs/how_to_guides/backup-restore) for details.
...
With `oras manifest index create`, `oras manifest index update`, you can easily assemble, update, distribute, and annotate multi-architecture images and artifacts across local environments and registries. Check out the user guidance [Create and Manage Multi-architecture Artifacts](/docs/how_to_guides/multiarch) for details.
...
Formatted output transforms ORAS from a simple terminal user tool to a **DevOps-friendly, integrable** developer tool. Check out the user guidance [Formatted output](/docs/how_to_guides/format_output) for details.
