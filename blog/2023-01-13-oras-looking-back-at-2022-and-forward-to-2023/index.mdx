...
ORAS provides an OCI registry client [ORAS CLI](https://oras.land/docs/CLI/installation) with functional-rich command sets that users can benefit from, while developers can build their own clients on top of one of the [ORAS client libraries](https://oras.land/docs/Client_Libraries/overview) including Golang and Python libraries.
