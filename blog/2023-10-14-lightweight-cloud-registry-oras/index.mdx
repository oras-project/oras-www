...
title: Lightweight Registry with Oras OCI Layouts and Cloud Object Storage
authors: [oras]
tags: [oras, community]
---
...
That code was already written in the "driver" for that "device", why repeat yourself in the program?
...
They are special components that convert media (CD, Tape, HDD, Controllers, etc.) into interface layers on which you can read/write files to.  
...
It is useful to think of COS as `key: data` databases instead of the file systems they pretend to be.  

Mounting one as a file system **should** be seen as "unintended functionality" by the COS and breaks some assumptions programs make about file systems.  
...
**You can't delete your artifacts!**

Currently[^1] you can't delete your local `oci-layout` artifacts with `oras manifest delete` or `oras blob delete`

The core reason for this is that normally it is the registry's responsibility to garbage collect blobs without any references.

It is still possible to just do `rm -rf s3/my-repo`, but that is probably not what you want to do.

[^1]: It is being worked on in [this issue](https://github.com/oras-project/oras-go/issues/454)

### Concurrency?

Same concurrency rules of your COS apply, generally [last-write-wins](https://en.wikipedia.org/wiki/Eventual_consistency).
...
Instead of integrating in your application code (hard dependency, vendor lock-in, complicates your interface: "how do I pass in creds?") you instead mount the COS in the container and then "just use the file system" (transferable, simple, external, interfaceable) 

There are some caveats to this approach and your application might leverage the capabilities of the Native COS api extremely well, but if it's just a simple interaction, why complicate things?


## Upgrading to Registry

### AWS Simple Storage Service -> ZOT
...
[Goofys](https://github.com/kahing/goofys) was the definitive champion utilizing the strengths of a caching file system with the powers of Cloud Object Storage, it was the fastest in the cloud and local.
...
Local performance:  
Test drive Goofys first, it made testing very difficult because it so effectively cached and reused object. Installing it was a pain though.  
`s3fs` installed easy but suffered around 512MiB, was bad at 1Gib and was awful at 2Gib.
...
Chunking the are fact did not lead to any substantial speed increases... Except for goofys, which showed an up to 2x speed increase!

![aws plot](./img/aws-plot.svg)

> AWS2023 removed the epel repos, I used a Debian for this test with the apt `s3fs` and the go `goofys`.
