...
As a tool to manage OCI artifacts, an ORAS library is expected to support the following functionalities:

### Must Have (Core Features)

An ORAS library must support the full set of registry APIs to enable interaction with OCI-compliant registires.
To enable artifact management, support of packaging and extraction of arbitrary files are necessary.

- **OCI Registry Client (Remote Artifact Store)**
   - Implement the APIs defined in the latest [OCI Distribution Spec](https://github.com/opencontainers/distribution-spec/blob/v1.1.1/spec.md).
   - Ensure compliance with the latest [OCI Image Spec](https://github.com/opencontainers/image-spec/blob/v1.1.1/spec.md).
   - Provide an authentication module that supports both basic and bearer authentication.
- **Artifact Packaging and Extraction**
   - Pack arbitrary files from the local file system into OCI artifacts.
   - Extract files from OCI artifacts to the local file system.

### Should Have (Recommended Features)

To be able to replicate artifacts, an ORAS library should support copy operations, including standard copy and extended copy.
It is also recommended to support widely recognized APIs defined in [Docker Registry HTTP API V2](https://distribution.github.io/distribution/spec/api/) but not in [OCI Distribution Spec](https://github.com/opencontainers/distribution-spec/blob/v1.1.1/spec.md), such as the [catalog API](https://distribution.github.io/distribution/spec/api/#listing-repositories).
In addition, it is recommended to support [OCI Image Layout](https://github.com/opencontainers/image-spec/blob/v1.1.1/image-layout.md) as a local artifact store. This is to enable importing/exporting of artifacts between remote registries and the local environment.
...
- **Extended Registry Client**
   - Support widely recognized APIs, such as the [Docker Registry HTTP API V2](https://distribution.github.io/distribution/spec/api/).
...
As docker is the most commonly used tool and docker credentials are widely recognized by various clients, it is helpful to support credential management thorugh docker config files and credential helpers.
...
TODO: table of check list for features implemented in each language