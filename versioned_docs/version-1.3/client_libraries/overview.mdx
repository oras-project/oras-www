...
This section defines a capability baseline for ORAS client libraries. Features are grouped by priority: Core (Must), Recommended (Should), and Optional (May). The intent is to set clear expectations for interoperability, artifact management, and ecosystem alignment. This baseline helps authors prioritize implementation (Core first) and helps users set expectations when selecting a library.

### Must Have (Core Features)

Core features are required for a library to be considered a compliant ORAS SDK. They enable essential interaction with OCI-compliant registries and basic artifact lifecycle operations.
...
Recommended features improve portability, reproducibility, and multi-registry workflows.
...
- **Extended Registry Client**
   - Support widely recognized APIs defined in [Docker Registry HTTP API V2](https://distribution.github.io/distribution/spec/api/) that are not part of the OCI Distribution Spec (e.g., repository catalog).
...
Optional features enhance usability and integration with existing container tooling.
...
Legend: ✅ Supported · ❌ Not Supported · ❓ Not Assessed
...
|---------|:--:|:------:|:----:|:--:|:----:|
| Standard Copy | ✅ | ❓ | ❓ | ❓ | ❓ |
| Extended Copy (Referrers) | ✅ | ❓ | ❓ | ❓ | ❓ |
| Extended Registry APIs (Catalog) | ✅ | ❓ | ❓ | ❓ | ❓ |
| OCI Image Layout: Read | ✅ | ❓ | ❓ | ❓ | ❓ |
| OCI Image Layout: Write | ✅ | ❓ | ❓ | ❓ | ❓ |
...
Updates to this table should accompany the implementation or documentation of the corresponding capability. Where a feature is not yet evaluated for a language, it remains marked ❓.